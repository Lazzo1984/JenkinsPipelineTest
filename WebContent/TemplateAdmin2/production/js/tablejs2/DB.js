var pre_proto = $.parseJSON($.ajax({
	...
	var ElettraArr = $.parseJSON($.ajax({
       // url:  "http://10.221.208.138:8095/project2/CertProd",
	...
	for(i =0;i < pre_proto.length; i++)
	{
		for(j = 0; j < ElettraArr.tableGeneral.length; j++)
		{
			if(pre_proto[i].Elettra == ElettraArr.tableGeneral[j][1])
			{
				proto.push(pre_proto[i]);
			}
		}
	}
...
var contatori = [{
            ...
            name: 'Vapt Severity',
            data: [
			{name: 'High',y: 0,color: 'red',selected: true},
			{name: 'Medium',y: 0,color: 'orange',selected: true},
			{name: 'Low',y: 0,color: 'yellow',selected: true},
            {name: 'Info',y: 0,color: 'cyan',selected: true}
			]
        }]
