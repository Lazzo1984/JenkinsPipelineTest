...
{
//if(splitted[o].toLowerCase().trim() === anal[j].Name.toLowerCase())// confronto secco
	if(splitted[o].toLowerCase().trim().includes(anal[j].Name.toLowerCase()))
{
if(anal[j].Cod == "3")
{VTag += (tRED + "<b>" + anal[j].Name + "</b>&nbsp;");}
else if(anal[j].Cod == "2")
{VTag += (tORA + "<b>" + anal[j].Name + "</b>&nbsp;");}
else if(anal[j].Cod == "1")
{VTag += (tYEL + "<b>" + anal[j].Name + "</b>&nbsp;");}
else if(anal[j].Cod == "0")
{VTag += (tCYA + "<b>" + anal[j].Name + "</b>&nbsp;");}
}else{}
...
// console.log(getCookie("scope"));
 var scope = getCookie("scope"); // FERRARIO.MASSIMO
 scope = scope.replace(/\"/g ,'');

 var ref = scope.split(".");
 	var _referente = proto[current].Responsabile;
	var _user = ref[0];
	var _rgx = new RegExp(_user + "\\s|,");
	var _match = _rgx.test(_referente);


if(_match || ref[1] == "ALL"){
 	 
var GetUrl = "/project2/ViewReport?Report=";
 		 
var tag2 = "<p>" + "<b>Credenziali/Configurazioni : </b>" + proto[current].Utenze + "</p>";

var tag1 = "<br><div style='clear:both;' class='linker'><a style='' href=" + GetUrl + proto[current].Elettra + " target=_blank ><span class='fa fa-external-link'></span>&nbsp;View Report</a></div>";

if(proto[current].attach >1)
{
	tag1 += "<div class='linker'><a style='' href=" + GetUrl + proto[current].Elettra + "_2" + " target=_blank ><span class='fa fa-external-link'></span>&nbsp;View Report 2</a></div>";
